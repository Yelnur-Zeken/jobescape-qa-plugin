...
- **PayPal** (`--paywall paypal`) — full automation as of 2026-04-30. The flag is appended to the **selling page URL** (not the funnel entry); funnel checkout is paid via the Solidgate iframe with the regular 4242 success card. The user is then registered as PayPal-cohort, and PayPal upsell versions (u15.1.x family) drive the **double_confirmation** mechanic on the upsell page: first Confirm Payment click → payment_intent_click event + "Confirm your payment" popup → second Confirm Payment in the popup → purchase_click + successful_purchase. Verified end-to-end on u15.1.3.
