...
- **PayPal-channel upsells** (u15.1.1, u15.1.2, u15.1.3, u13.0.1, u15.3.1) — **no special paywall flag required**. The version itself is designated for PayPal cohort server-side, and the upsell page renders the **double_confirmation** mechanic in its own UI. To test: walk the funnel normally with `--paywall solidgate` (default), pay with 4242, then visit `?upsell_version=u15.1.x` — the executor's mechanic-aware `doBuy` handles the popup automatically (first click → payment_intent_click + "Confirm your payment" popup → second click in popup → purchase_click + successful_purchase). The `--paywall paypal` flag is accepted as a clarity-alias but is functionally identical to `--paywall solidgate` for funnel walking. Verified on u15.1.3.
